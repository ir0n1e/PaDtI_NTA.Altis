...
if (isnil "IL_Variables") then
{
	...
	IL_Supported_Vehicles_All = IL_Supported_Vehicles_C130J + IL_Supported_Vehicles_MH9 + IL_Supported_Vehicles_MOHAWK + IL_Supported_Vehicles_KAMAZ + IL_Supported_Vehicles_HEMTT + IL_Supported_Vehicles_VAN + IL_Supported_Vehicles_OFFROAD;

	// Vehicles with the ability to dropping the load on the parachute
	IL_Para_Drop_Vehicles = IL_Supported_Vehicles_MOHAWK + IL_Supported_Vehicles_C130J;//["I_Heli_Transport_02_F"];
	...
	IL_Supported_Cargo_Veh_C130J = IL_Supported_Cargo_Veh_HEMTT + IL_Supported_Strider + IL_Supported_Hunter + IL_Supported_Ifrit;
...
//	END IL_Init_Veh
	...
	IL_Init_Box =
	{
		...
		{
			_obj setVariable["slots", 6, true];
			if (_obj_type == "B_G_Offroad_01_armed_F") then
			{
		...
		{
			{
				...
				else
				{
					...
					if ((_box_num > _slot_num) && !_done) then
					{
						...
						//_obj_type = (typeOf _v);
						if (((_obj_type in IL_Supported_Vehicles_HEMTT) || (_obj_type in IL_Supported_Vehicles_KAMAZ) || (_obj_type in IL_Supported_Vehicles_MOHAWK) || (_obj_type in IL_Supported_Vehicles_C130J) || (_obj_type in IL_Supported_Vehicles_OFFROAD) || (_obj_type in IL_Supported_Vehicles_VAN)) && (_doors == "B")) then
						{
...
if (_obj_main_type in IL_Supported_Vehicles_C130J) then
{
	...
	if (_obj_main_type == "C130J") then
	{
		...
		"('Cargo' in (assignedVehicleRole _this)) && (vehicle _this == _target) && (((getPosATL _target) select 2) >= IL_Para_Drop_ATL) && (_target animationPhase 'ramp_bottom' < 0.1) && (_target getVariable 'usable_ramp')"
		];
...
if (_obj_main_type in IL_Supported_Vehicles_OFFROAD) then
{
	...
	"(_target getVariable 'box_num' < 0) && ((driver _target == _this) || (((_this in (nearestObjects[ _target modelToWorld [0,-4.5,0], [], IL_SDistL + IL_SDistL_Heli_offset])) && (_target getVariable 'can_outside')))) && (_target getVariable 'can_load') && (abs(speed _target) <= IL_LU_Speed)"
	];
	_obj_main addAction [
	"<t color=""#0000ff"">Enable loading from outside</t>",
	{
