...
_ehF = _vehicle addEventHandler ["Fired",{
	if ((_this select 6) distance (getmarkerpos "Shootingrange") < 50) exitwith {};
	if(PO3_protector_enable) then {
		_p = _this select 6;
		if(
			_p distance (getMarkerPos "west_1") < 500
			...
			if !(player getvariable "haveBWA3" && {player getvariable "haveAGM"}) then {
				...
				player setvariable ["shootingatbase", true, true];
		}else{
			_p spawn {
				waitUntil {
					if(
						_this distance (getMarkerPos "west_1") < 500
