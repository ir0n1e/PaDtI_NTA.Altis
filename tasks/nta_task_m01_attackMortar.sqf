...
PO3_TASK__POS	= getmarkerpos "Artistellung_patrol1";
...
_pos = [20997.2,9166.9,0.00188446];
_object = createVehicle ["O_HMG_01_high_F", _pos, [], 0, "CAN_COLLIDE"];
_object setDir 250;
_object setPosATL _pos;
...
_object = createVehicle ["O_HMG_01_high_F", _pos, [], 0, "None"];
_object setDir 290;
_object setPosATL _pos;
...
_objects = [[20581.5,8918.79,36.4941], "O_HMG_01_high_F", _side, 236]  call NTA_fnc_vehicles_emplacement;

PO3_TOTAL_UNITS = PO3_TOTAL_UNITS + (_objects select 0);
PO3_TOTAL_VEHICLES = PO3_TOTAL_VEHICLES + (_objects select 1);

_objects = [[20579.2,9022.65,40.059], "O_HMG_01_high_F", _side, 303]  call NTA_fnc_vehicles_emplacement;
...
_grp = [_marker, _side, (configFile >> "CfgGroups" >> "East" >> "OPF_F" >> "Infantry" >> "OIA_InfSquad")] Call BIS_fnc_spawnGroup;
//_null=[leader _grp,_marker, "prefix:", house, "randomup","max:",10, "showmarker"] execVM "UPS.sqf";
_null=[leader _grp,"Artistellung","random","fortify","SPAWNED"] execVM "scripts\UPSMON.sqf";
{
	PO3_TOTAL_UNITS pushBack _x;
} forEach (units _grp);
...
_grp = [_marker, _side, (configFile >> "CfgGroups" >> "East" >> "OPF_F" >> "Infantry" >> "OI_SniperTeam")] Call BIS_fnc_spawnGroup;
_null=[leader _grp,"Artistellung_sniper","random","SPAWNED"] execVM "scripts\UPSMON.sqf";
{
	PO3_TOTAL_UNITS pushBack _x;
} forEach (units _grp);
...
_grp = [_marker, _side, (configFile >> "CfgGroups" >> "East" >> "OPF_F" >> "Infantry" >> "OI_reconPatrol")] Call BIS_fnc_spawnGroup;
_null=[leader _grp,"Artistellung_patrol1","random","SPAWNED"] execVM "scripts\UPSMON.sqf";
{
	PO3_TOTAL_UNITS pushBack _x;
} forEach (units _grp);
...
_grp = [_marker, _side, (configFile >> "CfgGroups" >> "East" >> "OPF_F" >> "Infantry" >> "OI_reconPatrol")] Call BIS_fnc_spawnGroup;
_null=[leader _grp,"Artistellung_patrol2","random","SPAWNED"] execVM "scripts\UPSMON.sqf";
{
	PO3_TOTAL_UNITS pushBack _x;
} forEach (units _grp);
