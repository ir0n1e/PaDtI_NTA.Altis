...
_actions = [format ["ParaDrop Menu - %1 Units", count apinsertgroupall], "Do It >>"] call AGM_Interaction_fnc_prepareSelectMenu;

{
	if (getNumber (configfile >> "CfgVehicles" >> _x >> "transportsoldier") > 8) then {
...
[
	_actions,
	{
		...
		if ((_this select 0) == "Chopper") then {
			...
			_this call NTA_fnc_airpatrol_agmOpenParaConfMenu;
		};
	},
	{
